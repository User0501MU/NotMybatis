...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (admin.isLogin_flag()) {
			...
			//▶「ページ転送先指定」request.getRequestDispatcher という方法で、移動先のページ（WEB-INF/jsp/customer_list.jsp）を指定しているんや
			//▶「ページに移動する」dispatcher?.forwardフォワード(request, response) を使って、今のページから指定したページ（customer_list.jsp）に移動するんや。
			...
			/*【補足】
			 * dispatcher（RequestDispatcher） は、リクエストとレスポンスを指定したページに転送するためのツールや。
			 * 使い方：getReqestDispatcher で転送先を決める。
			 * forward でリクエストとレスポンスを転送して、新しいページを表示する。*/
